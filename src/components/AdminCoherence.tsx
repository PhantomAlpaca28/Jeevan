...
// Import the generated premium 3D assets to render real assets in the admin view
// @ts-ignore
import fraudRadarImg from "../assets/images/3d_fraud_radar_1781988055512.jpg";
// @ts-ignore
import blockchainCubesImg from "../assets/images/3d_blockchain_cubes_1781988068342.jpg";
...
export default function AdminCoherence() {
  const [validators, setValidators] = useState<ValidatorNode[]>([
    ...
    { name: "Sovereign Self Gateway", region: "VitalTwin_Local", latencyMs: 1, syncHeight: 620491, status: "ONLINE" }
  ]);
    ...
    <div id="admin-security-coherence" className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          <div id="admin-security-holographic-monitor" className="relative w-full rounded-2xl border border-slate-900 bg-[#040c16]/90 overflow-hidden flex flex-col justify-between p-6 hover:border-emerald-500/10 transition-all shadow-xl">
            <div className="absolute inset-x-0 top-0 h-16 bg-gradient-to-b from-red-500/5 to-transparent pointer-events-none" />
            ...
            <div className="z-10 flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse shadow-[0_0_4px_rgba(239,68,68,0.5)]" />
                <span className="font-mono text-[10px] tracking-widest text-slate-400">LEDGER_PROTECTION_MATRIX</span>
              </div>
              <span className="font-mono text-[9px] text-red-400 bg-red-950/25 border border-red-900/30 px-2.5 py-0.5 rounded-full uppercase font-bold leading-none">
                Sentinel Online
              </span>
            </div>

            {/* Real 3D Fraud Radar asset display */}
            <div className="flex flex-col items-center justify-center relative min-h-[300px] my-2 z-10">
              {/* Red warning scan radar lines */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 border border-red-500/10 rounded-full animate-ping pointer-events-none" />
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 border border-red-500/20 rounded-full animate-pulse pointer-events-none" />

              <div className="relative w-44 h-44 flex items-center justify-center overflow-hidden hover:scale-[1.04] transition-all duration-500 rounded-2xl shadow-lg">
                <img
                  src={fraudRadarImg}
                  alt="3D Fraud Warning Radar"
                  className="w-full h-full object-contain pointer-events-none drop-shadow-[0_0_24px_rgba(239,68,68,0.15)]"
                  referrerPolicy="no-referrer"
                />
              </div>

              <div className="text-center mt-4">
                <span className="text-[10px] uppercase tracking-wider font-mono text-red-400 leading-none">THREAT DETECTION RADAR</span>
                <p className="text-[9px] text-slate-500 mt-0.5">Real-time Artificial Intelligence Security Sensor</p>
              </div>
            </div>
            ...
            <div className="z-10 grid grid-cols-3 bg-[#05111d] p-3.5 rounded-xl border border-slate-900 gap-2.5 text-center leading-none">
              <div className="flex flex-col items-center justify-center border-r border-slate-900">
                ...
                <div className="text-[8px] font-mono text-slate-500 mt-1">Membrane Protection</div>
              </div>
              ...
              <div className="flex flex-col items-center justify-center border-r border-slate-900 col-auto">
                ...
                <span className="text-sm font-semibold tracking-tight text-white font-mono">
                  {totalTrafficRequests} <span className="text-[8px] text-emerald-400 font-semibold">REQS</span>
                </span>
                <div className="text-[8px] font-mono text-slate-500 mt-1">Sovereign Queries</div>
              </div>
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                <div className="text-[8px] font-mono text-slate-500 mt-1">Globally Replicated</div>
              </div>
            </div>
          </div>
          ...
          <GlassCard glowColor="none" hoverable={false} className="p-4 bg-slate-950/40 border-slate-900">
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                disabled={isAuditing}
                className="w-full py-3 rounded-xl bg-gradient-to-r from-red-500/10 to-pink-500/5 hover:from-red-500/20 hover:to-pink-500/15 border border-red-500/25 hover:border-red-500/45 text-red-400 font-mono text-[10px] uppercase font-bold tracking-wider transition-all cursor-pointer flex items-center justify-center gap-2"
              >
              ...
              <AnimatePresence>
                {auditMessage && (
                  <motion.div
                    ...
                    exit={{ opacity: 0 }}
                    className="p-3 bg-red-950/20 border border-red-900/30 text-red-400 font-mono text-[9.5px] rounded-lg flex items-center gap-2"
                  >
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          <GlassCard glowColor="purple" hoverable={false} className="p-5 border-slate-900 bg-[#040c16]/90">
            ...
            {/* Blockchain cubes asset banner */}
            <div className="mb-4 bg-[#05111d] rounded-xl overflow-hidden border border-slate-900 p-3.5 flex items-center gap-4">
              <div className="w-16 h-16 shrink-0 rounded-lg overflow-hidden border border-purple-500/20">
                <img
                  src={blockchainCubesImg}
                  alt="3D Blockchain Sync Blocks"
                  className="w-full h-full object-cover pointer-events-none"
                  referrerPolicy="no-referrer"
                />
              </div>
              <div className="leading-normal">
                <span className="font-mono text-[8px] text-purple-400 font-bold block uppercase tracking-wider">LATEST VALIDATOR HEIGHT</span>
                <strong className="text-white text-sm font-display block select-all">BLOCK #620,491</strong>
                <p className="text-[9.5px] text-slate-500 leading-tight">Consensus synced across global quantum secure regions.</p>
              </div>
            </div>
              ...
              {validators.map((item) => (
                ...
                >
                  <div className="flex justify-between items-start leading-none gap-2">
                    ...
                    <span className="text-[8px] bg-red-950/20 border border-red-900/20 text-red-400 px-2 py-0.5 rounded font-bold uppercase tracking-wider shrink-0 leading-none">
